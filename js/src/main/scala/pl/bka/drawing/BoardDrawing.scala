    ...
    def drawComponent(component: Component, compIndex: Int): Unit = {
      ...
      component.cType match {
        ...
        case Capacitor(_, bipolar, _) =>
          val Seq(holePos1, holePos2) = holes.map(holePosition).sortBy(_._1)
          val (centerX, centerY) = ((holePos1._1 + holePos2._1) / 2, Seq(holePos1._2, holePos2._2).min - (0.3 * holeStep).toInt)
          DirectDrawing.drawLine(holePos1, (centerX - capacitorSize._1 / 2, centerY), 2)
          DirectDrawing.drawLine(holePos2, (centerX + capacitorSize._1 / 2, centerY), 2)
          DirectDrawing.drawCapacitorBody(component.name.value, (centerX, centerY))
        case _ => ()
      }
    }
